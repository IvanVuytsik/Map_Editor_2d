...
ROWS = 32
MAX_COLS = 256
TILE_SIZE = SCREEN_HEIGHT // ROWS *3
...
bg_sky = pygame.image.load("WorldEditor/SkyBG0.png").convert_alpha()
bg_sky = pygame.transform.scale(bg_sky, (int(SCREEN_WIDTH) * 2, (int(SCREEN_HEIGHT) * 2)))
bg_mountains = pygame.image.load("WorldEditor/MountainsBG0.png").convert_alpha()
bg_mountains = pygame.transform.scale(bg_mountains, (int(SCREEN_WIDTH * 2), (int(SCREEN_HEIGHT))))
bg_forest = pygame.image.load("WorldEditor/bg_forest0.png").convert_alpha()
...
for i in range(len(img_list)):
    tile_button = WorldButton(SCREEN_WIDTH + (50 * button_col) + 30, 50 * button_row + 20, img_list[i], 0.8)
...
for i in range(len(img_assets_list)):
    asset_button = WorldButton(SCREEN_WIDTH + (50 * button_col) + 5, 50 * button_row + 5, img_assets_list[i], 0.8)
    constructs_button_list.append(asset_button)
    button_col += 1
    if button_col == 2:
        button_row += 1
        button_col = 0
...
for i in range(len(img_trees_list)):
    tree_button = WorldButton(SCREEN_WIDTH + (50 * button_col) + 5, 50 * button_row + 5, img_trees_list[i], 0.8)
...
for i in range(len(img_plants_list)):
    plant_button = WorldButton(SCREEN_WIDTH + (50 * button_col) + 5, 50 * button_row + 5, img_plants_list[i], 0.8)
...
while run:
    ...
    draw_bg()
    draw_grid()
    draw_surface(world_trees, img_trees_list)
    draw_world()
    draw_surface(world_constructs, img_assets_list)
    draw_surface(world_nature, img_plants_list)
...
#-----------------------------------------Panel---------------------------------------
    ...
    if layer == 0:
        ...
        asset_count = 0
        for asset_count, i in enumerate(constructs_button_list):
            if i.draw(screen):
                current_asset = asset_count
            pygame.draw.rect(screen, red, constructs_button_list[current_asset].rect, 3)
    elif layer == 2:
        tree_count = 0
